...
var color = Color.fromHex("233067") as Color;

var rainbowtiger   = MaterialSystem.getMaterialBuilder().setName("rainbowtiger ").setColor(color).build();

rainbowtiger .registerParts(["gear", "plate", "nugget", "ingot", "rod", "dust"] as string[]);

var molten = rainbowtiger  .registerPart("molten").getData();
molten.addDataValue("temperature", "400");
molten.addDataValue("luminosity", "10");

var block = rainbowtiger  .registerPart("block").getData();
block.addDataValue("hardness", "5");
block.addDataValue("resistance", "30");
block.addDataValue("harvestTool", "pickaxe");

var ore = rainbowtiger .registerPart("ore").getData();
...
var armor = rainbowtiger .registerPart("armor").getData();
